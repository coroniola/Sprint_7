...
class TestCourierAuthorize:
    ...
    def test_authorize_non_existent_courier(self):
        ...
        expected_message = ExpectedMessages.COURIER_LOGIN_ACCOUNT_404_MESSAGE
        response = requests.post(Urls.LOGIN_COURIER, data=payload)
        assert response.status_code == 404 and  response.json().get('message') == expected_message
    ...
    def test_authorize_without_login(self,  registration_courier):
        ...
        expected_message = ExpectedMessages.COURIER_LOGIN_INCOMPLETE_DATA_MESSAGE
        response = requests.post(Urls.LOGIN_COURIER, data=payload)
